...
test("a character example never stacks a second face on the first", () => {
  // "Add a face" puts a face in the homepage box, so these tiles regularly
  // meet text that already has one. Replacing that end is the useful
  // reading; stacking is what produced "(◕‿◕) (◕‿◕) hi (≧▽≦)".
  for (const ex of FancyText.CHAR_EXAMPLES) {
    const out = FancyText.applyCharExample(ex, "(=^･ω･^=) hi (≧▽≦)");
    assert.ok(!/\(=\^･ω･\^=\)\s*\(/.test(out) && !/\)\s*\(≧▽≦\)\s*\(≧▽≦\)/.test(out),
      ex.id + " left a doubled face: " + out);
    // Its own output fed back in is a fixed point, not a taller pile.
    assert.strictEqual(FancyText.applyCharExample(ex, FancyText.applyCharExample(ex, "hi")),
      FancyText.applyCharExample(ex, "hi"),
      ex.id + " stacks its character when it meets its own output");
  }
});
...
test("a box holding only a face still has its face used as text", () => {
  ...
  // tile would render nothing at all.
  for (const ex of FancyText.CHAR_EXAMPLES) {
    assert.ok(FancyText.applyCharExample(ex, "(◕‿◕)").includes("(◕‿◕)"),
      ex.id + " ate the only token in the box");
  }
});
